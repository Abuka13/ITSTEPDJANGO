...
async def post_detail_get(request: Request):
    ...
    return templates.TemplateResponse("detail.html", {"request": request, "post": new_dict, "comments": comments_json})
...
@app.post("/like", response_class=RedirectResponse)
async def count_likes(request: Request):
    number_likes = 0
    old_number = number_likes
    number_likes+=1
    ...
    utils.db_query_sqlite(f"""UPDATE likes SET count_likes={number_likes} WHERE count_likes = {old_number}""",)

    return RedirectResponse(url=app.url_path_for("/detail"), status_code=303)
